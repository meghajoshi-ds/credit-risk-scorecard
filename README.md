...
│   ├── 06_out_of_time.ipynb        # Phase 10: out-of-time validation, PSI
│   ├── 07_fair_lending.ipynb       # Phase 11: disparate impact review
...
Run order: notebooks 01 → 07. Notebook 02 writes the clean CSV that 03 and 04
depend on.
...
Every result above rests on a **random** split, which this write-up previously
listed as the project's biggest weakness. Credit models are not used that way: a
lender trains on loans already originated and scores applicants arriving next
quarter. A random split puts loans from the same month on both sides of the
line, so the model gets credit for conditions it would not know about in
production.

The original extract had no origination date. `notebooks/06_out_of_time.ipynb`
uses a dated extract (396,030 loans, 2007&ndash;2016) to answer the question
directly: **how much did the random split flatter the model?**

### The answer

| Split | AUC | Gini | KS | Test set |
|---|---|---|---|---|
...
### The bigger finding: calibration drifts, ranking holds

Training on 2007&ndash;2013 and applying the model forward:

| Vintage | AUC | Actual default rate | Mean predicted |
|---|---|---|---|
| 2014 | 0.695 | **23.1%** | 17.5% |
| 2015 | 0.696 | **24.9%** | 18.1% |

Discrimination barely moves, but the model **under-predicts risk by 6&ndash;7
percentage points**. It was trained on vintages defaulting at 15&ndash;16% and
applied to ones defaulting at 23&ndash;25%.

A lender monitoring AUC alone would have seen a healthy model while
systematically under-pricing. This is precisely what out-of-time validation
exists to catch, and it is invisible to a random split.

### A trap in the data, caught before it mattered

2016 reports a 13.1% default rate against 24.9% in 2015. Credit quality did not
...
vintages too young to have one. **2016 was dropped**; using it as the test set
would have produced a better-looking, meaningless result.

### Also excluded: `grade` and `sub_grade`

This extract carries Lending Club's internal risk grade directly. It is the
*output of their risk model*, making it even more blatant leakage than the
interest rate derived from it.

### Population stability

PSI between the training window and 2015 explains *why* performance moved,
rather than just reporting that it did. `verification_status` shifted most
(PSI 0.18, moderate); everything else stayed below 0.10. In production, PSI is
what triggers a rebuild long before anyone waits to observe defaults.

## Phase 11: Fair lending review

Under ECOA and Regulation B, a US lender cannot deploy a scorecard without
testing for **disparate impact**. A model that uses no prohibited attribute can
still disadvantage a protected group through proxies. This is a legal
requirement, not an optional extra, and it is almost always missing from
portfolio projects.

Note the shape of the problem: **this dataset contains no protected attribute at
all**, because lenders are generally barred from collecting race for consumer
credit. So the analysis works through a proxy, and geography is the standard
one, since residential segregation makes location correlate with race. That is
also the mechanism behind historical redlining.

### Adverse Impact Ratio (the four-fifths rule)

Approval rate for each group, divided by the rate for the most-approved group.
Below **0.80** is the conventional flag. Tested across cut-offs from the 10th to
50th score percentile rather than at one convenient point.

| Region | Worst AIR across cut-offs |
|---|---|
| West | 1.000 |
| Northeast | 0.949 |
| South | 0.941 |
...
### Does the model actually use geography?

A counterfactual test: hold every characteristic fixed, change only the region.

Score moves by **4.2 points on average** (median 3.4) against a working range of
about 140 &mdash; roughly 3%, consistent with `region` having the lowest
Information Value of any characteristic. But the **maximum swing is 14.8
points**, so for some applicants location is worth over 10% of the range. The
average understates the tail, and in a real review the tail is what gets
examined.

### The real proxy risk is not geography

An honest audit names the characteristics that could carry a protected
attribute. The one a compliance function would question hardest is
**`longest_credit_length`**: it is close to a linear function of age, and age is
an explicitly prohibited basis under ECOA. It is a legitimate risk
characteristic *and* an age proxy simultaneously. Resolving that is a legal
judgement rather than a modelling one, which is exactly why fair lending review
sits outside the modelling team.

`annual_inc` is the largest proxy risk by size of effect, though ability to
repay is the strongest available business justification.

### What this cannot establish

Passing the four-fifths rule is a screen, not a clearance:

- **No protected attribute is observed.** A real review uses **BISG** (Bayesian
  Improved Surname Geocoding), the CFPB's own method, combining surname and
  census-tract demographics. That needs surnames and tract-level addresses,
  neither of which this data has.
- **State is too coarse.** Redlining operates at neighbourhood level; state
  averages wash out the disparities that matter.
- **Only approve/decline is tested**, not pricing, limits or marketing reach.

Documenting that gap is the correct outcome. The value is in having looked, and
in being precise about what looking could and could not show.

## Known limitations

Stated plainly, because a model's limitations are part of its documentation:

1. **Probabilities need recalibrating for the deployment period.** Phase 10
   shows the model under-predicts by 6&ndash;7 percentage points when applied
   forward, because it was trained on vintages defaulting at 15&ndash;16% and
   applied to ones at 23&ndash;25%. Ranking holds; the absolute level does not.
